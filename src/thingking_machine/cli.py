...
def options_and_arguments():
    ...
    parser = argparse.ArgumentParser(
        description="Thingking-Machine thinks for you about the meanings.",
        epilog="Example:  thingking-machine input_text.txt > output_text.txt"
...
def run():
    """
    $ text | thingking-machine                        # Accepts text from the pipe
    $ echo "...<text>..." | thingking-machine         #

    $ thingking-machine multilogue.txt new_turn.txt    # ...or files.
    """
    ...
    try:
        ...
        syslog.openlog(
            ident="thingking-machine",
            logoption=syslog.LOG_NDELAY,
            facility=syslog.LOG_USER
        )
        # Signal (single line less than 4096 only!)
        syslog.syslog(syslog.LOG_INFO, f"thingking-machine: {utterance}.")
        syslog.closelog()
